...
This private function converts a key/value pair from the
[`docments`](https://fastcore.fast.ai/docments.html#docments) structure
into the `dict` that will be needed for the schema.
...
[`mk_tool`](https://fastcore.fast.ai/funccall.html#mk_tool) is the
inverse of
[`get_schema`](https://fastcore.fast.ai/funccall.html#get_schema) — it
creates a callable Python function from a JSON schema tool definition.
This is useful for MCP clients where tools are defined as schemas but
need to be called as regular Python functions.
...
[`get_schema`](https://fastcore.fast.ai/funccall.html#get_schema) on
[`mk_tool`](https://fastcore.fast.ai/funccall.html#mk_tool)’s output
reproduces the schema it was built from. This matters wherever a remote
tool re-enters a local toolchain, e.g. an MCP client’s bound tools
passed to a chat API. Parameter descriptions ride on the rebuilt
signature as `Annotated` metadata, which is how
