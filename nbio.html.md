...
Jupyter’s wire protocol accepts Python values that the standard JSON
encoder does not. In particular, rich binary outputs such as images are
base64 encoded; dates, iterables, and numeric subclasses follow
Jupyter’s Python conversion rules.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L43"
...
``` python
packed = dumps({"image/png": b"raw"}, default=jupyter_json_default)
test_eq(loads(packed), {"image/png": "cmF3"})
packed
```
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L86"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L89"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L130"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L137"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L145"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L159"
target="_blank" style="float:right; font-size:smaller">source</a>
...
merges a notebook’s frontmatter from three sources, lowest precedence
first: the notebook’s own `metadata.nbdev` mapping, its first markdown
cell, and its first raw cell anywhere. A cell that is *entirely* a
literal `---` block parses as YAML (so `---`-prefixed prose never
half-parses), and a markdown cell that isn’t one contributes `# title`,
`> description`, and `- key: value` lines instead. Content beating
metadata mirrors how `directives` reads a cell (comments win over the
meta form), so both scopes tell one precedence story. Metadata values
pass through verbatim: the `'true'`→`''` bare-directive normalization
belongs to cell directives, where bareness must be canonical, and has no
meaning in a mapping where every key carries a value. It takes anything
whose `cells` items carry `cell_type` and `source`, which includes
aidialog/solveit dialogs (their messages duck-type as cells; ones
without a `metadata` attr simply contribute nothing from that source);
`strvals=True` keeps every YAML scalar a string apart from
[`frontmatter`](https://fastcore.fast.ai/xtras.html#frontmatter)’s bool
exception, and malformed YAML in a literal block raises.
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L184"
target="_blank" style="float:right; font-size:smaller">source</a>
...
*Frontmatter from `nb`, merged lowest-to-highest from its
`metadata.nbdev` mapping (values verbatim), first markdown cell (literal
`---` block, or `# title` synthesis), and first raw cell anywhere*

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L170"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L165"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L235"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L279"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L276"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L303"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L312"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L320"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L329"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L352"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L380"
target="_blank" style="float:right; font-size:smaller">source</a>
...
[`diff_cells`](https://fastcore.fast.ai/nbio.html#diff_cells) compares
two versions of a cell sequence – an open notebook against its file on
disk, a dialog against an edited copy – and yields the edits that turn
one into the other. Items are aligned by `id` (any objects with `.id`
work, not just cells), and each edit is a *block*: a contiguous run of
deletions, insertions, or in-place changes, so an applier can handle a
whole run at once and inserted items keep their order. Identity
questions ride the ids; whether an aligned pair *changed* is a content
question, answered by comparing `key(item)` on each side.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L406"
target="_blank" style="float:right; font-size:smaller">source</a>
...
*Yield block `CellEdit`s that turn `a` into `b`, aligned by item `id`*

Five cells, then a copy with one deleted, one edited, and one inserted.
Edits arrive in `a`-order: `old` and `new` are lists (parallel for
`change`), and an insert’s `idx` anchors the block in `a` – the new
items belong after `a[idx-1]`, or at the front when `idx` is 0:
...
The default `key` compares whole items, which for cells means their full
dict – outputs included. Pass a projection to name the content that
matters: with `key=attrgetter('source')`, an outputs-only difference is
no edit at all. The same parameter serves hosts whose items don’t
compare by content at all (aidialog messages, say, whose `==` checks
ids), by projecting each item to its comparable form:
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L459"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L444"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L505"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L468"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L539"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L560"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L556"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L552"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L545"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L565"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L581"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L590"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L608"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L638"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L646"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L665"
target="_blank" style="float:right; font-size:smaller">source</a>
...
``` python
def cells2xml(
    cells, wrap:functools.partial=functools.partial(<function ft at 0x7f2b366ceb60>, 'nb'), ids:bool=True,
    incl_out:bool=True, **kw
):
```
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L659"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L671"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L717"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L731"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L737"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L749"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L763"
target="_blank" style="float:right; font-size:smaller">source</a>
...
``` python
def prev_line(
    txt, maxlen:int=180, pre:str='', sep:str=''
):
```

*One-line preview `pre`+`sep`+`txt` capped at `maxlen`, blank-line runs
shown as `¶`; a cut line carries `txt`’s size before `sep` and ends with
the count of characters missing*

[`prev_line`](https://fastcore.fast.ai/nbio.html#prev_line) is the
one-line row shared by
[`CellRow`](https://fastcore.fast.ai/nbio.html#cellrow) and aidialog’s
message previews. A row that fits shows its whole text. A row that was
cut tells the reader two different things in two different places: the
node’s size sits in the prefix, before the separator, and the elision at
the end counts the characters missing:

``` python
test_eq(prev_line('abc\n\ndef', 20, 'id', ':'), 'id:abc¶def')
r = prev_line('x'*100, 20, 'id', ':')
test_eq(r, 'id[100]:'+'x'*7+'…[93]')
```

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L780"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L771"
target="_blank" style="float:right; font-size:smaller">source</a>
...
``` python
def CellRow(
    c, maxlen:int=180, kind:str='match'
):
```
...
A long cell’s row shows the cell’s size before its separator and how
much of the source was cut at the end:

``` python
row = repr(CellRow(mk_cell('x'*300), maxlen=40))
assert '[300]:' in row and len(row)==40
row
```
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L796"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L789"
target="_blank" style="float:right; font-size:smaller">source</a>
...
def summary_nb(
    ...
    maxlen:int=180, # Maximum source characters per line
):
```
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L785"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L801"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L825"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L840"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L852"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L865"
target="_blank" style="float:right; font-size:smaller">source</a>
...
cells. Whether a selected cell then participates follows the `eval`
cascade: the cell’s own `eval:` directive wins, else the notebook-level
`eval` directive, else the `default_eval` argument. Cells named by id
always run — `#| eval: false` means “not by default”, not “never” — and
`ignore_eval=True` skips the cascade entirely, like run-all in Jupyter:

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L910"
target="_blank" style="float:right; font-size:smaller">source</a>
...
given; cells named in `msgids` always run, others follow the `eval`
cascade*

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L901"
target="_blank" style="float:right; font-size:smaller">source</a>
...
*Does `cell` participate in a non-interactive run? Decided by its `eval`
directive, or `default` when the directive is absent or unrecognized; a
cell tagged `skip-execution` (nbclient’s skip tag) never participates*

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L894"
target="_blank" style="float:right; font-size:smaller">source</a>
...
*Participation default for cells without their own `eval` directive: the
notebook-level `eval` directive in frontmatter mapping `fm` if given,
else `default_eval`*
...
Directive lookalikes inside strings are not directives, metadata-form
directives count like comment-form ones, and a cell tagged
`skip-execution` never participates: that tag is nbclient’s own “never
run” marker, so honoring it keeps `nbdev-test` and
[`select_cells`](https://fastcore.fast.ai/nbio.html#select_cells) in
agreement with what `jupyter execute` would do.
...
assert not does_cell_eval(tc, True)  # the tag wins whatever the default: what nbclient skips, nothing runs
```

The full participation rule is the `eval` cascade: the cell’s own
directive, else the notebook-level `eval` directive (any
[`nb_frontmatter`](https://fastcore.fast.ai/nbio.html#nb_frontmatter)
source — frontmatter block, `# title` list line, or notebook
`metadata.nbdev`), else the `default_eval` argument.
[`fm_default_eval`](https://fastcore.fast.ai/nbio.html#fm_default_eval)
resolves the notebook level, recognizing
[`true`](https://fastcore.fast.ai/basics.html#true)/`True`/`false`/`False`
as bools or strings and ignoring anything else;
[`does_cell_eval`](https://fastcore.fast.ai/nbio.html#does_cell_eval)
applies the cell level. Passing `default_eval=False` turns a run opt-in
— only cells marked `#| eval: true` participate — which is how template
fill runs a dialog’s cells while batch testing keeps the run-by-default
policy:
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L939"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L973"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/nbio.py#L964"
target="_blank" style="float:right; font-size:smaller">source</a>
