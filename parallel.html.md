...
`parallel(f, items)` applies `f` to each item using a process pool. Set
`threadpool=True` to use threads. Set `n_workers` to control
concurrency, or to 0 for serial execution when debugging. Optional
arguments include a `progress` bar, a `pause` between starts, and
`return_exceptions` to collect errors.
[`parallel_async`](https://fastcore.fast.ai/parallel.html#parallel_async)
runs work concurrently with asyncio.

`@threaded` runs a function in a background thread and returns the
thread. Read its `result` attribute after it finishes. Set
`process=True` to run in a process.
[`startthread`](https://fastcore.fast.ai/parallel.html#startthread) and
[`startproc`](https://fastcore.fast.ai/parallel.html#startproc) start
their decorated functions immediately.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L32"
...
Pass `daemon=True` when the thread or process must not prevent its
parent from exiting. This is useful for background services such as
webservers.
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L54"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L61"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L81"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L90"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L109"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L131"
target="_blank" style="float:right; font-size:smaller">source</a>
...
Use `pause` to stagger starts, for example when making requests to a
webserver. Its value is in seconds. Set `threadpool=True` to use
threads.
...
yields `(index, result)` pairs as tasks complete. The index identifies
the input item. It creates all tasks at the start and limits concurrent
execution to `n_workers`.

By default, exiting the generator cancels unfinished tasks and waits for
cancellation. This includes errors and early closure. Pass
`cancel_on_exit=False` to leave those tasks running.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L156"
target="_blank" style="float:right; font-size:smaller">source</a>
...
Results arrive in completion order. Here the later items sleep less and
finish first. Sort by index to recover input order:
...
Use `contextlib.aclosing` when you might stop iteration early. It closes
the generator on `break` and waits for cleanup:
...
collects results into a dictionary keyed by input index. The
dictionary’s iteration order is completion order. It accepts the same
options as
[`parallel_async_gen`](https://fastcore.fast.ai/parallel.html#parallel_async_gen).
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L179"
target="_blank" style="float:right; font-size:smaller">source</a>
...
With `cancel_on_error=True`, the first failure cancels the remaining
tasks.
[`parallel_async`](https://fastcore.fast.ai/parallel.html#parallel_async)
waits for cancellation before raising the original exception.
...
A fire-and-forget task needs a reference to keep it alive and somewhere
to report failures when no caller awaits it.
[`bg_task`](https://fastcore.fast.ai/parallel.html#bg_task) keeps the
reference until the task finishes and prints exception tracebacks to
stderr by default. Pass `on_err` to handle the exception yourself.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/parallel.py#L193"
target="_blank" style="float:right; font-size:smaller">source</a>
...
Use `on_err` to report failures somewhere other than stderr, such as a
server’s error logger. This example collects exceptions in a list:
