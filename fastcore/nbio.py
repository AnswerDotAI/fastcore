...
# %% auto #0
__all__ = ['rich_mimes', 'NbCell', 'dict2nb', 'read_nb', 'mk_cell', 'new_nb', 'nb2dict', 'nb2str', 'write_nb', 'validate_cell',
           'validate_nb', 'preferred_out', 'mk_stream', 'mk_result', 'mk_display', 'mk_error', 'concat_streams',
           'preferred_msg_out', 'render_output', 'render_outputs', 'render_text', 'render_media', 'cell2xml',
           'cells2xml', 'Notebook']
...
# %% ../nbs/13_nbio.ipynb #eb675539
rich_mimes = ('image/', 'audio/', 'video/', 'application/pdf')
def render_media(outputs, mimes=rich_mimes):
    "Extract rich media from `outputs` as `(mime, data)` pairs, base64-encoding raw bytes"
    if (not isinstance(outputs, (list,tuple))) or (outputs and not isinstance(outputs[0],dict)): return []
    res = ((m, _join(d)) for o in outputs if o.get('output_type') in ('execute_result','display_data')
           for m,d in o.get('data',{}).items() if m.startswith(mimes) and '+' not in m)
