...
- [`FixSigMeta`](https://fastcore.fast.ai/meta.html#fixsigmeta)
  preserves signature introspection (i.e. tab completion in IDEs) when
  inheritance through a `__new__` or a metaclass `__call__` would
  otherwise obscure it; the other metaclasses here all inherit from it.
- [`PrePostInitMeta`](https://fastcore.fast.ai/meta.html#prepostinitmeta)
  runs `__pre_init__` and `__post_init__` automatically around
  `__init__` (without having to call them yourself).
- [`AutoInit`](https://fastcore.fast.ai/meta.html#autoinit) is a
  [`PrePostInitMeta`](https://fastcore.fast.ai/meta.html#prepostinitmeta)
  mixin that calls `super().__init__` for you.
- [`NewChkMeta`](https://fastcore.fast.ai/meta.html#newchkmeta) makes
  `C(x)` return `x` unchanged when `x` is already a `C` and no other
  args are passed (how [`L`](https://fastcore.fast.ai/foundation.html#l)
  avoids re-wrapping an
  [`L`](https://fastcore.fast.ai/foundation.html#l)).
- [`BypassNewMeta`](https://fastcore.fast.ai/meta.html#bypassnewmeta)
  casts an object of another class (`_bypass_type`) to this class
  without copying it.

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L39"
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L59"
target="_blank" style="float:right; font-size:smaller">source</a>
...
[`use_kwargs`](https://fastcore.fast.ai/meta.html#use_kwargs)/[`use_kwargs_dict`](https://fastcore.fast.ai/meta.html#use_kwargs_dict)
replace `**kwargs` in a signature with named params from an explicit
list or dict, and
[`funcs_kwargs`](https://fastcore.fast.ai/meta.html#funcs_kwargs) lets a
class accept method overrides as constructor kwargs. The one used
everywhere in fastai code is `@delegates`: decorating `wrapper` with
`@delegates(base)` replaces `**kwargs` in `wrapper`’s shown signature
with `base`’s keyword parameters, so introspection and tab completion
reveal the real options. Only params with defaults are copied over;
`keep=True` retains `**kwargs`; `but=` excludes names; and on a class
with no argument it delegates to the superclass:

------------------------------------------------------------------------

<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L93"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L106"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L119"
target="_blank" style="float:right; font-size:smaller">source</a>
...
This resolves the actual underlying functions regardless of whether you
pass a function, class, or `None`.

When `to` is a plain function, it’s used directly. When it’s a class,
`__init__` is unwrapped. When `None`, it pulls `__init__` from both the
class and its parent — the class-to-parent delegation case.
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L155"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L178"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L184"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L222"
target="_blank" style="float:right; font-size:smaller">source</a>
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L228"
target="_blank" style="float:right; font-size:smaller">source</a>
...
Configurable decorators normally come with a tax: every author must
hand-roll the “was I called with the function, or with the
configuration?” dispatch, and every hand-rolling is a chance to get it
wrong. [`metadec`](https://fastcore.fast.ai/meta.html#metadec) pays that
tax once. Write the decorator in its natural shape - the function first,
config as keyword-only params - and both application forms work. The
keyword-only rule is what makes dispatch unambiguous by *call shape*
rather than by sniffing argument types (a lone positional argument is
always the decoratee, even when a config value happens to be callable),
so [`metadec`](https://fastcore.fast.ai/meta.html#metadec) enforces it
at decoration time. fastcore’s own
...
[`athreaded`](https://fastcore.fast.ai/aio.html#athreaded) are all
defined with it.
...
A positional config param would make `@d(value)` ambiguous, so it fails
at definition time rather than misdispatching at use:
...
<a
href="https://github.com/AnswerDotAI/fastcore/blob/main/fastcore/meta.py#L239"
target="_blank" style="float:right; font-size:smaller">source</a>
...
A `**kwargs` collector on `fn` survives the splice, moving after the
wrapper’s own keyword-only params, since Python accepts a collector only
at the very end:
