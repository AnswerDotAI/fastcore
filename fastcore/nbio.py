...
# %% ../nbs/13_nbio.ipynb #9520a858
CellEdit = namedtuple('CellEdit', 'op old new idx', defaults=(None,None,None))

def diff_cells(
    a, # Old sequence of items carrying `.id` (cells, messages, ...)
    b, # New sequence
    key=noop, # Projection to comparable data, for change detection
):
    "Yield block `CellEdit`s that turn `a` into `b`, aligned by item `id`"
    sm = SequenceMatcher(None, [o.id for o in a], [o.id for o in b])
    for tag,i1,i2,j1,j2 in sm.get_opcodes():
        xa,xb = a[i1:i2],b[j1:j2]
        if tag=='equal':
            chg = [(oa,ob) for oa,ob in zip(xa,xb) if key(oa)!=key(ob)]
            if chg: yield CellEdit('change', old=[o for o,_ in chg], new=[o for _,o in chg])
        else:
            if xa: yield CellEdit('delete', old=xa)
            if xb: yield CellEdit('insert', new=xb, idx=i1)
